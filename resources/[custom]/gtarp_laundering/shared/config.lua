-- ============================================================================
-- gtarp_laundering/shared/config.lua — engine-agnostic tunables (Tier 1,
...
-- summon police, and an evidence trail (gtarp_evidence v2) so a laundering
-- run can become a case.
--
-- NOT counterfeit. gtarp_counterfeit's `counterfeit_cash` is FAKE money that
...
-- RP, same as gtarp_chopshop's drop point. Server-side distance check against
...
-- gtarp_chopshop/gunrunning/ransom) eventguard's Config.Events doesn't cover
...
-- ---------------------------------------------------------------------------
-- Evidence (gtarp_evidence v2 frozen exports). A flagged run opens/updates a
