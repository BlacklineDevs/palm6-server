-- ============================================================================
-- gtarp_chopshop/server/main.lua
--
...
-- pattern as gtarp_ransom (kidnap) and gtarp_gunrunning (ballistics).
--
...
-- cannot, no export exists) write into it. `gtarp_chopshop_stolen` is its
-- own independent, persistent, queryable registry; a future gtarp_mdt
...
local function dbg(msg)
    if Config.Debug then print('[gtarp_chopshop] ' .. msg) end
end
...
local function cmdReportStolen(src, args)
    ...
    pcall(function()
        existing = MySQL.single.await(
            "SELECT id FROM gtarp_chopshop_stolen WHERE plate = ? AND status = 'active'", { plate })
    end)
    if existing then
    ...
    local ok = pcall(function()
        MySQL.insert.await(
            [[INSERT INTO gtarp_chopshop_stolen
...
local function cmdSellStolen(src)
    ...
    local ok = pcall(function()
        saleId = MySQL.insert.await(
            [[INSERT INTO gtarp_chopshop_sales
    ...
    if stolenRow then
        ...
        pcall(function()
            MySQL.update.await(
                "UPDATE gtarp_chopshop_stolen SET status = 'resolved', resolved_at = NOW() WHERE id = ? AND status = 'active'",
                { stolenRow.id })
        end)
        ...
        if Bridge.ResourceStarted('gtarp_evidence') then
            pcall(function()
                ...
                evidenceCaseId = exports.gtarp_evidence:EnsureCase(incidentKey, 'Stolen vehicle sold to chop shop', 'gtarp_chopshop')
                if evidenceCaseId then
                    exports.gtarp_evidence:AppendEntry(evidenceCaseId, 'chopshop_sale', {
                        plate = plate, vehicle_class = class, payout = payout,
                        owner_citizenid = stolenRow.owner_citizenid,
                    }, 'gtarp_chopshop')
                    exports.gtarp_evidence:LinkSuspect(evidenceCaseId, cid, nil)
                end
            end)
        end
        if evidenceCaseId then
            pcall(function()
                MySQL.update.await('UPDATE gtarp_chopshop_sales SET evidence_case_id = ? WHERE id = ?',
...
AddEventHandler('onResourceStart', function(resource)
    ...
    pcall(function()
        local r = MySQL.single.await(
            "SELECT COUNT(*) AS n FROM gtarp_chopshop_stolen WHERE status = 'active' AND expires_at > NOW()")
        activeReports = r and tonumber(r.n) or 0
    end)
    pcall(function()
        local r = MySQL.single.await('SELECT COUNT(*) AS n FROM gtarp_chopshop_sales')
        totalSales = r and tonumber(r.n) or 0
    end)
    print(('[gtarp_chopshop] shop open — %d active stolen report(s), %d sale(s) all-time'):format(activeReports, totalSales))
end)
